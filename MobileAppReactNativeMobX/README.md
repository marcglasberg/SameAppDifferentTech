...
In the `index.js` file, we inject a few dependencies using the `inject` function.

The inject function can be found in the `inject.tsx` file.
...
When you use `inject({})` as shown above, it creates a new store (please check the `inject.tsx` file).
...
In the `dao.ts` file, we define the `Dao` as an abstract class or interface:
...
In file `SimulatedDao.ts` you can see how it returns a list of available stocks,
...
In the same `SimulatedDao.ts` file, look at the methods `listenToStockPriceUpdates()` and `stopStockPriceUpdates()` to
...
It's defined in file `RunConfig.ts`.
...
Check file `ABTesting.ts` to see how this is implemented.
...
For instance, `anotherConfiguration` is defined in the `inject.tsx` file like this:
...
(see `Playground.tsx`) instead of the usual app.
...
Please check the `Storage.ts` file.
...
The specific scenario which I have implemented in the `StorageManager.ts` file is this:
...
5. In `App.tsx` there is a mechanism implemented with the `handleAppShutdown()` function,
...
In file `Fonts.ts` we define the fonts that we use in the app, taken from the design system:
...
In file `Layout.tsx` I define a few simple components that substitute some of the uses of `<View>`:
...
example, `Stock.test.ts` and `utils.test.ts`.

<br>

### Testing the store

Then, I test the sub-stores that are part of the main store, in `UiStore.test.ts`, `Portfolio.test.tsx`
and `AvailableStock.test.tsx`:
...
Then I test the infrastructure classes, in `RunConfig.test.ts`, `Storage.test.ts`, `StorageManager.test.ts`
and `Dao.test.ts`.
...
File `AvailableStock.mixed.tsx` defines the `AvailableStock_Mixed` component,
...
<a href="https://mobx.js.org/the-gist-of-mobx.html">don't need to use</a> the container/view pattern:
...
1. The Container
   ...
   File `AvailableStock.container.tsx` contains `AvailableStockContainer`, which has no UI.
   ...
   Let's see how to test the **container** in file `AvailableStocks.container.test.ts`.
...
2. The View
   ...
   File `AvailableStock.view.tsx` contains `AvailableStockView`, which does not access the store directly.
...
This folder contains a `hooks` directory containing file `AvailableStock.hook.tsx` which is **not used** in the app.

Compare it with files `AvailableStock.container.tsx` and `AvailableStock.view.tsx`.
...
to the component, as seen in file `AvailableStock.hook.tsx`.
