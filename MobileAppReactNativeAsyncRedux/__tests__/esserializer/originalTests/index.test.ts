...
import SuperClassA from './fixture/SuperClassA';
import ClassA from './fixture/ClassA';
import ClassB from './fixture/ClassB';
import ClassC from './fixture/ClassC';
import MyObject from './fixture/MyObject';
...
describe('Test deserialize', () => {
  test('can deserialize array', () => {
    const serializedText = '[{"age":88,"*type":"Person"},{"property1":"First","property2":"Second","*type":"MyObject"}]';
    expect(Array.isArray(ESSerializer.deserialize(serializedText, [Person, MyObject]))).toBe(true);
  });

  test('can deserialize complex text', () => {
    const serializedText = '{"_hobby":"football","*type":"ClassB","toy":{"_height":29,"*type":"ClassC"},"friends":[{"_name":"Old man","age":88,"*type":"ClassA"},{"_height":54,"*type":"ClassC"},"To be or not to be"]}';
  ...
  test('can throw meaningful exception if class is missing', () => {
    ...
    }).toThrow('Class "ClassC" not found during deserialization. ' +
      'You must register it with: "ESSerializer.registerClass(ClassC);"');
  });

  test('can deserialize for function style class definition', () => {
    const serializedText = '{"age":77,"*type":"Person"}';
  ...
  test('can serialize and deserialize Set', () => {
    ...
    const serializedText = ESSerializer.serialize(objToSerialize);
    expect(ESSerializer.deserialize(serializedText).set).toStrictEqual(new Set([42, 55, 'Hello', {
      a: 1,
      b: 2
    }, { a: 1, b: 2 }]));
  });
  ...
  test('can serialize and deserialize Intl.NumberFormat', () => {
    ...
    const serializedText = ESSerializer.serialize(objToSerialize);
    expect(ESSerializer.deserialize(serializedText).inf).toStrictEqual(new Intl.NumberFormat('de-DE', {
      style: 'currency',
      currency: 'EUR'
    }));
  });
...
describe('Test clear operation', () => {
  test('can clear registered classes', () => {
    ...
    }).toThrow('Class "ClassB" not found during deserialization. ' +
      'You must register it with: "ESSerializer.registerClass(ClassB);"');
  });
});
